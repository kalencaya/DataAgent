...
/**
 * MaxCompute JDBC DDL 执行器实现
 */
@Service
public class MaxComputeJdbcDdl extends AbstractJdbcDdl {
	...
	public List<TableInfoBO> showTables(Connection connection, String schema, String tablePattern) {
		...
		try {
			...
			for (int i = 1; i < resultArr.length; i++) {
				...
				String[] tableNames = resultArr[i][0].split("\n");
				...
				for (String tableName : tableNames) {
					...
					if (ArrayUtils.getLength(split) == 1) {
						tableNameSet.add(split[0]);
					} else if (ArrayUtils.getLength(split) == 2) {
						tableNameSet.add(split[1]);
					}
				}
			}

			tableNameSet.forEach(tableName -> tableInfoList.add(TableInfoBO.builder().name(tableName).build()));
		}
		catch (SQLException e) {
			throw new RuntimeException(e);
		}
	...
	public List<TableInfoBO> fetchTables(Connection connection, String schema, List<String> tables) {
		...
		for (String tableName : tables) {
			try {
				String sql = String.format(sql_template, schema, tableName);
				String[][] resultArr = SqlExecutor.executeSqlAndReturnArr(connection, sql);

				String tableComment = "";
				for (int i = 1; i < resultArr.length; i++) {
					if (resultArr[i].length >= 2 && "table_comment".equalsIgnoreCase(resultArr[i][0].trim())) {
	...
	public BizDataSourceTypeEnum getDataSourceType() {
		return BizDataSourceTypeEnum.MAX_COMPUTE;
	}
